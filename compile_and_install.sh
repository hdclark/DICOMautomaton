...
MINIMAL="no" # minimal mode: disable optional deps, limit optimization, use CMake to fetch non-standard deps.
...
while getopts "b:d:i:uncmh" opt; do
    ...
    h|*)
        ...
        printf "          : Default: '%s'\n" "${CLEANBUILD}"
        printf "\n"
        printf " -m       : Minimal mode; disable all optional dependencies and use CMake to\n"
        printf "          : automatically fetch the non-standard upstream support libraries (Ygor,\n"
        printf "          : YgorClustering, Explicator). This results in the fastest possible build\n"
        printf "          : with the fewest external dependencies. Uses the generic (direct-install)\n"
        printf "          : code path.\n"
        printf "          : Default: '%s'\n" "${MINIMAL}"
        printf "\n"
        exit 0
