...
int main(int argc, char *argv[]){
  ...
  const char filename_invalid[] = "Some_Non_Existant_File.dcm";
  ...
  if( !s.Scan( filenames ) )  return 1;

  //s.Print( std::cout );


  if( s.IsKey( filename ) ) std::cout << "INFO:" << filename << " is a proper Key for the Scanner (this is a DICOM file)" << std::endl;
    

  if( !s.IsKey( filename_invalid ) )   std::cout << "INFO:" << filename_invalid << " is not a proper Key for the Scanner (this is either not a DICOM file or file does not exist)" << std::endl;
