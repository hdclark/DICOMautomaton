...
void DrawGLScene(){
    ...
    if(DICOM_data.Has_Dose_Data()){
        ...
        glBegin(GL_QUADS);
        for(auto l_it = DICOM_data.dose_data.begin(); l_it != DICOM_data.dose_data.end(); ++l_it){
...
/*
  ...
  */ 
    ...
    //Data dumping/interception.
        ...
        for(auto it = Filenames_In.begin(); it != Filenames_In.end(); ++it) ShuttleOut << "# " << *it << std::endl;
...
//These are fairly common options. Run the program with -h to see them formatted properly.
    ...
    do{
        ...
        switch(next_options){
            ...
            case 'i':
                if(optarg != nullptr) Filenames_In.push_back(optarg);
                break;
