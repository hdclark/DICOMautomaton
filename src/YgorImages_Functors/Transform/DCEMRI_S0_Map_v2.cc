...
bool DCEMRIS0MapV2(planar_image_collection<float,double>::images_list_it_t  local_img_it,
    ...
    if(N < 2){
        YLOGWARN("This routine require two or more images with distinct flip angles to produce an S0 map. "
    ...
    for(auto img_it : overlapping_imgs){
        ...
        if(!FA) YLOGERR("Image is missing 'FlipAngle', which is needed to compute S0. Cannot continue");
        ...
        if(!RT) YLOGERR("Image is missing 'RepetitionTime', which is needed to compute S0. Cannot continue");
...
*/
    ...
    UpdateImageDescription( std::ref(*local_img_it), "S0 Map" );
