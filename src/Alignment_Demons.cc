                ...
                for(int64_t col = 0; col < N_cols; ++col){
                    ...
                    for(int64_t k = -kernel_radius_z; k <= kernel_radius_z; ++k){
                        ...
                        if(img_k >= 0 && img_k < N_imgs){
                            const double val = temp_y.images[img_k].value(row, col, chnl);
                            if(std::isfinite(val)){
...
compute_gradient(const planar_image_collection<float, double> & img_coll){
    ...
    for(size_t img_idx = 0; img_idx < img_coll.images.size(); ++img_idx){
        const auto &img = img_coll.images[img_idx];
        
        planar_image<double, double> grad_img;
        grad_img.init_orientation(img.image_plane);
        ...
        for(int64_t row = 0; row < N_rows; ++row){
            for(int64_t col = 0; col < N_cols; ++col){
                ...
                if(N_imgs > 1){
                    if(img_idx > 0 && img_idx < N_imgs - 1){
                        const double val_prev = img_coll.images[img_idx - 1].value(row, col, 0);
                        const double val_next = img_coll.images[img_idx + 1].value(row, col, 0);
...
AlignViaDemons(AlignViaDemonsParams & params,
    ...
    try {
        ...
        for(const auto &img : stationary.images){
            ...
            def_img.init_orientation(img.image_plane);
        ...
        for(int64_t iter = 0; iter < params.max_iterations; ++iter){
            ...
            for(size_t img_idx = 0; img_idx < stationary.images.size(); ++img_idx){
                const auto &fixed_img = stationary.images[img_idx];
                const auto &warped_img = warped_moving.images[img_idx];
                const auto &grad_img = gradient.images[img_idx];
                
                planar_image<double, double> update_img;
                update_img.init_orientation(fixed_img.image_plane);
