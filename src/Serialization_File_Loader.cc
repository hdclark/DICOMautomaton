//Serialization_File_Loader.cc - A part of DICOMautomaton 2026. Written by hal clark.
//
// This program loads data from Ygor-serialized Drover archives.
//
...
bool Load_From_Serialization_Files( Drover &DICOM_data,
                                    std::map<std::string,std::string> & /* InvocationMetadata */,
                                    const std::string & /* FilenameLex */,
                                    std::list<std::filesystem::path> &Filenames ){

    // This routine will attempt to load Ygor-serialized Drover files. Files that are not successfully loaded are not
    // consumed so that they can be passed on to the next loading stage as needed.
    //
    ...
    for(const auto &fn : Filenames_Copy){
        ...
        Drover A;
        const bool res = Common_Deserialize_Drover(A, fn);
        if(res){
