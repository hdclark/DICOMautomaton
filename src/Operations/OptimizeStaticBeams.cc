...
Drover OptimizeStaticBeams(Drover DICOM_data, OperationArgPkg OptArgs, std::map<std::string,std::string> /*InvocationMetadata*/, std::string FilenameLex){
    ...
    while(iap_it != DICOM_data.image_data.end()){
...

        {
            ...
            if(!Modality || (Modality.value() != "RTDOSE")){
                throw std::runtime_error("Asked to compute dose on a non-RTDOSE Image_Array. Refusing to continue.");
                // Note: there are legitimate reasons to want to do this. Enable if necessary.
            }
        }


/////////////////////////////
// Replace with the modern, two-stage version?

        // Get voxel doses corresponding to the ROIs.
        ...
        for(const auto &av : ud.accumulated_voxels){
            ...
            for(const auto &D_voxel : av.second){
                voxels.push_back(D_voxel);
            }
        }
/////////////////////////////

// Trim the rest to suit...
