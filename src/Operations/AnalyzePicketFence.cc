...
Drover AnalyzePicketFence(Drover DICOM_data, OperationArgPkg OptArgs, std::map<std::string,std::string> /*InvocationMetadata*/, std::string FilenameLex){
    ...
    auto CSVFileName = OptArgs.getValueStr("CSVFileName").value();
...
*/
                ...
                try{
                    ...
                    if(CSVFileName.empty()){
                        CSVFileName = Get_Unique_Sequential_Filename("/tmp/dicomautomaton_evaluatenpf_", 6, ".csv");
                    }
                    const auto FirstWrite = !Does_File_Exist_And_Can_Be_Read(CSVFileName);
                    std::fstream FO_pf(CSVFileName, std::fstream::out | std::fstream::app);
                    if(!FO_pf){
