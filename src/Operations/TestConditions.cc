//TestConditions.cc - A part of DICOMautomaton 2026. Written by hal clark.
...
OperationDoc OpArgDocTestConditions(){
    ...
    out.desc =
        ...
        " If any condition is not satisfied, the operation evaluates to false."
        "\n\n"
    ...
    out.args.back().desc = "A list of conditions to evaluate, where each function represents a single condition."
                           ...
                           "maximum pixel value in the selected Image_Array(s). `min` or `max` can be non-numeric to"
                           " disable that check (e.g., 'x'). The tolerance applies to both `min` and `max`.\n"
                           "\n\n"
    ...
    out.args.back().examples = { "image_array_count(1)",
                                 ...
                                 "pixel_minmax(0, 70, 0.25)",
                                 "pixel_minmax(disabled, 70, 0.1)",
                                 "pixel_minmax(0, disabled, 0.1)",
...
bool TestConditions(Drover &DICOM_data,
    ...
    for(const auto &pf : pfs){
        ...
        // Drover object count conditions
        if(std::regex_match(pf.name, regex_ia_count)){
            if( (pf.parameters.size() != 1UL) && pf.parameters.at(0).number ){
                throw std::invalid_argument("image_array_count requires exactly 1 argument (expected count)");
            }
            const auto expected = static_cast<int64_t>(pf.parameters.at(0).number.value());
        ...
        // Pixel value conditions 
            ...
            if( (pf.parameters.size() < 2UL) || (pf.parameters.size() > 3UL) ){
                throw std::invalid_argument("pixel_minmax requires 2 or 3 arguments"
                                            " (min, max, optional tolerance)");
            }
            const auto expected_min = pf.parameters.at(0).number;
            const auto expected_max = pf.parameters.at(1).number;
            if(!expected_min && !expected_max){
                throw std::invalid_argument("pixel_minmax requires at least one numeric"
                                            " bound (min or max)");
            }
            const auto tolerance = (pf.parameters.size() == 3UL) ? pf.parameters.at(2).number.value()
                                                                 : default_tolerance;
            if(tolerance < 0.0){
