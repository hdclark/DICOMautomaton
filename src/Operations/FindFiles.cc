...
OperationDoc OpArgDocFindFiles(){
    ...
    out.desc = 
        ...
        " Optionally, each located item is recorded as a row in a table (see TableOutputMethod).";
    ...
    out.notes.emplace_back(
        "Name and path filters use case-insensitive regular expressions via Compile_Regex()."
    );
    ...
    out.args.back().desc = "Controls whether files, directories, or both should be located."
                           ...
                           " 'files' matches only regular files.";
...
bool FindFiles(Drover& DICOM_data,
    ...
    const auto passes_filters = [&](const std::filesystem::path& p, int64_t depth) -> bool {
        ...
        if( is_reg_file && !get_files) return false;
        if(!is_dir      && !is_reg_file) return false;

        const auto fname = p.filename().string();
        if(name_re      && !std::regex_match(fname, *name_re     )) return false;

        const auto fpath = p.string();
        if(wholename_re && !std::regex_match(fpath, *wholename_re)) return false;

        if(newer_mt_threshold){
            const time_mark mod_tm( get_mod_time_t(p) );
            if(mod_tm <= *newer_mt_threshold) return false;
        }
        return true;
    };
