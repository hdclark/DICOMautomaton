...
OperationDoc OpArgDocDitherImages(){
    ...
    out.args.back().expected = true;
    out.args.back().examples = { "0.0", "-1.0", "-1000.0" };
    ...
    out.args.back().expected = true;
    out.args.back().examples = { "1.0", "1000.0", "3000.0" };
    ...
    out.args.back().expected = true;
    out.args.back().examples = { "0.5", "128.0", "500.0" };
...
bool DitherImages(Drover &DICOM_data,
    ...
    const auto Low = std::stof( OptArgs.getValueStr("Low").value() );
    const auto High = std::stof( OptArgs.getValueStr("High").value() );
    const auto Threshold = std::stof( OptArgs.getValueStr("Threshold").value() );
