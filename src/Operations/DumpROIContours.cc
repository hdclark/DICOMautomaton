...
std::list<OperationArgDoc> OpArgDocDumpROIContours(void){
    ...
    out.back().desc = "A filename (or full path) in which to append contour dump data.";
                      " Leave empty to dump to generate a unique temporary file.";
    out.back().default_val = "";
    out.back().expected = true;
    out.back().examples = { "", "/tmp/somefile", "localfile.dump", "derivative_data.dump" };
...
Drover DumpROIContours(Drover DICOM_data, OperationArgPkg OptArgs, std::map<std::string,std::string> /*InvocationMetadata*/, std::string FilenameLex){
    ...
    if(DumpFileName.empty()){
        DumpFileName = Get_Unique_Sequential_Filename("/tmp/dicomautomaton_dumproicontours_", 6, ".dump");
    }
    ...
    {
        std::ofstream FO(DumpFileName, std::fstream::out | std::fstream::app); 

        const auto dump_vert = [&](long int family_num, 
                                   long int contour_num, 
                                   const vec3<double> &p) -> void {
                FO << "Vertex: "
                   << "family " << family_num << " "
                   << "contour " << contour_num << " "
                   << p.x << " " << p.y << " " << p.z
        ...
        for(const auto &cc_ref : cc_ROIs){
            long int contour = 0; //This records which contour each vertex belongs to.
            for(const auto &c : cc_ref.get().contours){
                for(const auto &p : c.points) dump_vert(family, contour, p);
                if(!c.points.empty() && c.closed) dump_vert(family, contour, c.points.front());
                ++contour;
            }
            ++family;
        }
