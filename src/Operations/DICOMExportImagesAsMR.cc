...
#include "DICOMExportImagesAsMR.h"
...
OperationDoc OpArgDocDICOMExportImagesAsMR(){
    ...
    out.args.back().desc = "The filename (or full path name) to which the DICOM files should be written."
                           " The file format is a gzipped-TAR file containing multiple MR-modality files.";
    out.args.back().default_val = "MRs.tgz";
    out.args.back().expected = true;
    out.args.back().examples = { "/tmp/MRs.tgz", "./MRs.tar.gz", "MRs.tgz" };
...
bool DICOMExportImagesAsMR(Drover &DICOM_data,
...

    {
        ...
        for(auto & iap_it : IAs){
            ...
            // This closure is invoked once per MR slice file. We simply add each to the TAR file.
            ...
            try{
                Write_MR_Images(*iap_it, file_handler, p);
            }catch(const std::exception &e){
                YLOGWARN("Unable to export Image_Array as DICOM MR-modality files: '" << e.what() << "'");
            }
        }
