...
Drover DumpTPlanMetadataOccurrencesToFile(Drover DICOM_data, OperationArgPkg OptArgs, std::map<std::string,std::string> /*InvocationMetadata*/, std::string /*FilenameLex*/){
    ...
    for(auto & tpp_it : TPs){
        ...
        // Plan metadata.
        for(const auto &md_pair : (*tpp_it)->metadata){
            ++(sset[md_pair.first][md_pair.second]);
        }

        // Beam metadata.
        for(const auto &abeam : (*tpp_it)->dynamic_states){
            for(const auto &md_pair : abeam.metadata){
                ++(sset[md_pair.first][md_pair.second]);
            }
        }
    }
    ...
    try{
        ...
        boost::interprocess::named_mutex mutex(boost::interprocess::open_or_create,
                                               "dicomautomaton_operation_dumptplanmetadataoccurrencestofile_mutex");
        boost::interprocess::scoped_lock<boost::interprocess::named_mutex> lock(mutex);

        if(FileName.empty()){
            FileName = Get_Unique_Sequential_Filename("/tmp/dicomautomaton_dumptplanmetadataoccurrencestofile_", 6, ".tsv");
        }
