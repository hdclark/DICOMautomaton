...
Drover SpatialBlur(Drover DICOM_data, OperationArgPkg /*OptArgs*/, std::map<std::string,std::string> InvocationMetadata, std::string /*FilenameLex*/){
    ...
    //This operation (spatial) blurs pixels in all available images.
    //
    // The implementation is currently basic and without many (any) options. It is hoped that it will be expanded with
    // different blurs and options like the supersampling operation was. However, the default should stay: in-plane
    // only, Gaussian, blur distance in terms of pixel widths, and all available images blurred.
    //
