...
//std::cout << "Unable to place." << std::endl;
    ...
    struct sf_game_t {
        ...
        double skier_y = 500.0;       // Skier y position (fixed on screen, world scrolls)
        double skier_size = 12.0;     // Skier triangle size
        
        double scroll_speed = 50.0;   // Initial downward scroll speed (pixels/sec)
        double max_scroll_speed = 300.0; // Maximum scroll speed
        double speed_increase_rate = 5.0; // Speed increase per second
        ...
        double tree_spawn_rate = 1.5;      // Average trees per second
        double rock_spawn_rate = 1.0;      // Average rocks per second  
        double jump_spawn_rate = 0.3;      // Average jumps per second
        double other_skier_spawn_rate = 0.2; // Average other skiers per second
    ...
    const auto reset_sf_game = [&](){
        ...
        sf_game.skier_y = sf_game.box_height * 0.85;
...
*/
        ...
        const auto display_sf_game = [&view_toggles,
            ...
            ImGui::SetNextWindowSize(ImVec2(win_width, win_height), ImGuiCond_FirstUseEver);
            ImGui::SetNextWindowPos(ImVec2(100, 100), ImGuiCond_FirstUseEver);
            ImGui::Begin("SkiFree", &view_toggles.view_skifree_enabled, flags );
            ...
            if(!sf_game.countdown_active && !sf_game.game_over){
                ...
                if((total_time - sf_game.last_tree_spawn) > (1.0 / sf_game.tree_spawn_rate)){
                    ...
                    sf_game_objs.back().pos = vec2<double>(rd_x(sf_game.re), sf_game.world_scroll_y - 50.0);
                    sf_game_objs.back().size = 15.0;
                ...
                sf_game_objs.erase(
                    std::remove_if(sf_game_objs.begin(), sf_game_objs.end(),
                        [&](const sf_game_obj_t &obj) -> bool {
                            ...
                            return screen_y > sf_game.box_height + 100.0;
                        }),
                    sf_game_objs.end()
                );
            }
            ...
            for(const auto &obj : sf_game_objs){
                ...
                if(obj.type == sf_obj_type_t::tree){
                    ...
                    // Draw jump as a brown/yellow ramp (triangle)
                    const auto c = ImColor(0.8f, 0.6f, 0.2f, 1.0f);
                    const auto p1 = ImVec2(obj_pos.x - obj.size, obj_pos.y + obj.size*0.3);
                    const auto p2 = ImVec2(obj_pos.x + obj.size, obj_pos.y + obj.size*0.3);
                    const auto p3 = ImVec2(obj_pos.x + obj.size, obj_pos.y - obj.size*0.3);
                    window_draw_list->AddTriangleFilled(p1, p2, p3, c);
                }else if(obj.type == sf_obj_type_t::other_skier){
                    // Draw other skier as a cyan triangle
            ...
            {
                std::stringstream ss;
                ss << "Score: " << sf_game.score << "  Speed: " << static_cast<int>(sf_game.scroll_speed);
