...
//std::cout << "Unable to place." << std::endl;
    ...
    struct lg_game_t {
        ...
        double spawn_interval = 2.5;
        double enemy_speed = 22.0;
...
*/
        ...
        // Lawn game display function.
        const auto display_lg_game = [&view_toggles,
                                      &lg_towers,
                                      &lg_enemies,
                                      &lg_projectiles,
                                      &lg_tokens,
            ...
            ImGui::SetNextWindowSize(ImVec2(win_width, win_height), ImGuiCond_FirstUseEver);
            ImGui::SetNextWindowPos(ImVec2(150, 150), ImGuiCond_FirstUseEver);
            ImGui::Begin("Lawn Game", &view_toggles.view_lawn_game_enabled, flags);
            ...
            tower_button("Sun (3)", lg_tower_type_t::Sun, 3);
            tower_button("Shooter (2)", lg_tower_type_t::Shooter, 2);
            tower_button("Blocker (1)", lg_tower_type_t::Blocker, 1);
            ImGui::Separator();
                ...
                if(rel_x >= 0 && rel_y >= 0 && rel_x <= lg_game.board_width && rel_y <= lg_game.board_height){
                    ...
                    if(!has_tower(lane, col)){
                        ...
                        switch(lg_game.selected_tower){
                            case lg_tower_type_t::Sun: cost = 3; break;
                            case lg_tower_type_t::Shooter: cost = 2; break;
                            case lg_tower_type_t::Blocker: cost = 1; break;
                        }
            ...
            if(lg_game.lives > 0){
                for(auto& enemy : lg_enemies){
                    ...
                    if(blocking_col >= 0){
                        for(auto& tower : lg_towers){
                            if(tower.lane == enemy.lane && tower.col == blocking_col){
                                target_tower = &tower;
                                is_blocked = true;
                                break;
                            }
                    ...
                    if(is_blocked && target_tower != nullptr){
                        ...
                        enemy.x -= lg_game.enemy_speed * dt;
                    }
                }
            }
            ...
            for(const auto& tower : lg_towers){
                ...
                const float radius = static_cast<float>(lg_game.cell_width * 0.25);
                ImU32 fill_col = 0;
                ImU32 border_col = 0;
                char label = '?';
                if(tower.type == lg_tower_type_t::Sun){
                    ...
                    label = 'S';
                    draw_list->AddCircleFilled(ImVec2(cx, cy), radius, fill_col);
                    draw_list->AddCircle(ImVec2(cx, cy), radius, border_col, 0, 2.0f);
                    ...
                    draw_list->AddConvexPolyFilled(tri, 3, fill_col);
                    draw_list->AddTriangle(tri[0], tri[1], tri[2], border_col, 2.0f);
                }else{
                    fill_col = ImColor(0.4f, 0.6f, 0.4f, 1.0f);
                    border_col = ImColor(0.2f, 0.35f, 0.2f, 1.0f);
                    label = 'B';
                    const int sides = 8;
                    ...
                    for(int i = 0; i < sides; ++i){
                        draw_list->AddLine(oct_pts[i], oct_pts[(i + 1) % sides], border_col, 2.0f);
                    }
                }
                char text_str[2] = { label, '\0' };
                const ImVec2 text_sz = ImGui::CalcTextSize(text_str);
                draw_list->AddText(ImVec2(cx - text_sz.x * 0.5f, cy - text_sz.y * 0.5f),
            ...
            for(const auto& enemy : lg_enemies){
                ...
                const float radius = static_cast<float>(lg_game.cell_width * 0.23);
                const ImU32 fill_col = ImColor(0.3f, 0.1f, 0.1f, 1.0f);
                const ImU32 border_col = ImColor(0.55f, 0.15f, 0.15f, 1.0f);
                draw_list->AddCircleFilled(ImVec2(ex, ey), radius, fill_col);
                draw_list->AddCircle(ImVec2(ex, ey), radius, border_col, 0, 2.0f);
                const char text_str[2] = { 'E', '\0' };
                const ImVec2 text_sz = ImGui::CalcTextSize(text_str);
                draw_list->AddText(ImVec2(ex - text_sz.x * 0.5f, ey - text_sz.y * 0.5f),
                                   ImColor(0.9f, 0.8f, 0.8f, 1.0f), text_str);

                const float bar_w = lg_game.cell_width * 0.5f;
                const float bar_h = 4.0f;
                const float bar_x = ex - bar_w * 0.5f;
                const float bar_y = ey - radius - 8.0f;
                const float hp_ratio = static_cast<float>(enemy.hp / enemy.max_hp);
                draw_list->AddRectFilled(ImVec2(bar_x, bar_y),
        ...
        try{
            if(!display_lg_game()) break;
        }catch(const std::exception &e){
            YLOGWARN("Exception in display_lg_game(): '" << e.what() << "'");
            throw;
        }


        // Display the Guitar Fret game.
        const auto display_gf_game = [&view_toggles,
                                      &gf_game,
                                      &t_gf_updated,
                                      &gf_lane_pressed,
                                      &gf_lane_scored,
        ...
        try{
            // Break from the main render loop if false is received.
            if(!display_gf_game()) break;
        }catch(const std::exception &e){
            YLOGWARN("Exception in display_gf_game(): '" << e.what() << "'");
            throw;
        }
