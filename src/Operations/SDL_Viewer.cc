...
//
            ...
            if( false
            &&  view_toggles.view_vector_sketching_enabled
            &&  image_mouse_pos.DICOM_to_pixels ){
                auto &slot = current_sketch_slot();
                auto &sketch = slot.history.current();
                ...
                &&  sketch_compatible ){
                    ...
                    if(const auto hovered_vertex = sketch.nearest_primitive_vertex(image_mouse_pos.dicom_pos, tol); hovered_vertex){
                ...
                if(sketch_compatible){
                    ...
                    for(std::size_t i = 0U; i < sketch.primitive_count(); ++i){
                        ...
                        if(const auto *vertex_primitive = dynamic_cast<const Sketch::vertex_primitive_t*>(primitive); vertex_primitive != nullptr){
                            const auto p = image_mouse_pos.DICOM_to_pixels(sketch.vertex(vertex_primitive->vertex));
                    ...
                    &&  !sketch_pending_primitive.points.empty() ){
                        ...
                        if(image_mouse_pos.mouse_hovering_image){
                            auto preview_point = image_mouse_pos.dicom_pos;
                        ...
                        const auto preview_plane = sketch.has_plane()
                                                ...
                                                : try_get_current_sketch_plane(disp_img_it);
                        ...
                        if(show_circle_hint){
                           ...
                           && sketch_hovered_primitive ){
                        imgs_window_draw_list->AddCircleFilled(image_mouse_pos.DICOM_to_pixels(image_mouse_pos.dicom_pos),
                                                               4.5f,
            ...
            if(false && view_toggles.view_vector_sketching_enabled){
                ...
                if(view_sketch_editor_enabled){
                    ...
                    }else if(ImGui::CollapsingHeader("Plane", ImGuiTreeNodeFlags_DefaultOpen)){
                        const auto plane_frame = slot.history.current().plane();
                        auto origin_x = plane_frame.origin.x;
                        auto origin_y = plane_frame.origin.y;
                        auto origin_z = plane_frame.origin.z;
                        auto row_x = plane_frame.row_unit.x;
                        auto row_y = plane_frame.row_unit.y;
                        auto row_z = plane_frame.row_unit.z;
                        auto col_x = plane_frame.col_unit.x;
                        auto col_y = plane_frame.col_unit.y;
                        auto col_z = plane_frame.col_unit.z;
                        bool plane_changed = false;
                        ImGui::SetNextItemWidth(100.0f);
                        plane_changed = ImGui::InputDouble("Origin X", &origin_x) || plane_changed;
                        ImGui::SameLine();
                        ImGui::SetNextItemWidth(100.0f);
                        plane_changed = ImGui::InputDouble("Origin Y", &origin_y) || plane_changed;
                        ImGui::SameLine();
                        ImGui::SetNextItemWidth(100.0f);
                        plane_changed = ImGui::InputDouble("Origin Z", &origin_z) || plane_changed;
                        ImGui::SetNextItemWidth(100.0f);
                        plane_changed = ImGui::InputDouble("Row X", &row_x) || plane_changed;
                        ImGui::SameLine();
                        ImGui::SetNextItemWidth(100.0f);
                        plane_changed = ImGui::InputDouble("Row Y", &row_y) || plane_changed;
                        ImGui::SameLine();
                        ImGui::SetNextItemWidth(100.0f);
                        plane_changed = ImGui::InputDouble("Row Z", &row_z) || plane_changed;
                        ImGui::SetNextItemWidth(100.0f);
                        plane_changed = ImGui::InputDouble("Col X", &col_x) || plane_changed;
                        ImGui::SameLine();
                        ImGui::SetNextItemWidth(100.0f);
                        plane_changed = ImGui::InputDouble("Col Y", &col_y) || plane_changed;
                        ImGui::SameLine();
                        ImGui::SetNextItemWidth(100.0f);
                        plane_changed = ImGui::InputDouble("Col Z", &col_z) || plane_changed;
                        if(plane_changed){
                            edit_current_sketch([&](Sketch &editable_sketch){
                                Sketch::plane_frame_t editable_plane;
                                editable_plane.origin = vec3<double>(origin_x, origin_y, origin_z);
                                editable_plane.row_unit = vec3<double>(row_x, row_y, row_z);
                                editable_plane.col_unit = vec3<double>(col_x, col_y, col_z);
                                editable_sketch.set_plane(editable_plane);
                            });
                        }
                    }
        ...
        try{
            ...
            image_mouse_pos_s image_mouse_pos{};
            image_mouse_pos.mouse_hovering_image = canvas_hovered;
            image_mouse_pos.image_window_focused = ImGui::IsWindowFocused();
            image_mouse_pos.image_window_hovered = ImGui::IsWindowHovered();
            if(canvas_hovered){
...
if( view_toggles.view_vector_sketching_enabled
            &&  image_mouse_pos.DICOM_to_pixels ){
                ...
                if( image_mouse_pos.mouse_hovering_image
                &&  sketch_compatible ){
                    const auto tol = sketch_canvas_selection_tolerance(image_mouse_pos);
                    if(const auto hovered_vertex = sketch.nearest_primitive_vertex(image_mouse_pos.dicom_pos, tol); hovered_vertex){
                        sketch_hovered_primitive = hovered_vertex->first;
                        sketch_hovered_vertex = hovered_vertex->second;
                    }else{
            ...
            ImGui::End();
            sketch_mouse_pos_s sketch_mouse_pos;
            sketch_mouse_pos.mouse_hovering_canvas = image_mouse_pos.mouse_hovering_image;
            sketch_mouse_pos.sketch_window_focused = image_mouse_pos.image_window_focused;
            sketch_mouse_pos.sketch_window_hovered = image_mouse_pos.image_window_hovered;
            sketch_mouse_pos.plane_pos = Sketch::projection_t{ plane_u, plane_v };
            sketch_mouse_pos.dicom_pos = image_mouse_pos.dicom_pos;
            sketch_mouse_pos.pixel_scale = image_mouse_pos.pixel_scale;
            sketch_mouse_pos.DICOM_to_pixels = image_mouse_pos.DICOM_to_pixels;
            sketch_mouse_pos_opt = sketch_mouse_pos;
            const std::optional<image_mouse_pos_s> image_mouse_pos_opt = image_mouse_pos;
            ...
            if(image_mouse_pos.image_window_focused || image_mouse_pos.image_window_hovered || canvas_hovered){
                    if( view_toggles.view_vector_sketching_enabled
                          &&  sketch_drag_state.anchor
