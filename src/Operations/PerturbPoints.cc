...
OperationDoc OpArgDocPerturbPoints(){
    ...
    out.args.back().desc = "The width of the axis-aligned cube (in DICOM units; mm) within which"
                           " points can be randomly shifted. The cube is centered on each point."
...
bool PerturbPoints(Drover &DICOM_data,
    ...
    if( !std::isfinite(Width) || (Width < 0.0) ){
        throw std::invalid_argument("Width must be finite and non-negative. Cannot continue.");
    }
    ...
    for(auto & pcp_it : PCs){
        ...
        // Early exit if no perturbation is needed.
        if(Width <= 0.0){
            YLOGINFO("Width is <= 0.0, not perturbing any points");
            continue;
        }
