//BoostSerializeDrover.h - A part of DICOMautomaton 2016. Written by hal clark.
...
#include "BoostSerializeDrover.h"


Drover
Boost_Serialize_Drover(Drover DICOM_data, std::map<std::string, std::string> /*InvocationMetadata*/, std::string /*FilenameLex*/){

    //This routine serializes the entire Drover class to a single file, making it easy to persist a snapshot of the
    //  data. 
    //
    // NOTE: Be aware that these dumps can be large, and, e.g., will not fit in /tmp/ in some cases. For example, an
    //       xml serialization of a greatly reduced dataset was 500MB raw which xz-compressed down to only 16MB.
    //

    const std::string out_fname("/tmp/boost_serialized_drover.binary");
    try{
        std::ofstream ofs(out_fname, std::ios::trunc | std::ios::binary);
