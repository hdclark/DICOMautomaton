...
OperationDoc OpArgDocSerializeDrover(){
    ...
    out.desc = 
        "This operation exports selected loaded state to a serialized XML format (Ygor) that can be loaded again later."
        " It is especially useful for suspending long-running operations with intermittent interactive sub-operations."
        " New archives are gzipped XML by default. Legacy Boost archives and binary/text archive variants are not"
        " supported.";
