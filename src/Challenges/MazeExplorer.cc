...
MazeExplorerGame::MazeExplorerGame(){
    maze_game.re.seed(std::random_device()());
    Reset();
}

void MazeExplorerGame::Reset(){
    maze_game.level_complete = false;
    maze_game.completion_time = 0.0;
    maze_game.is_jumping = false;
    maze_game.jump_height = 0.0;
    maze_game.jump_velocity = 0.0;
    maze_game.walk_phase = 0.0;
...
bool MazeExplorerGame::InBounds(int64_t x, int64_t y) const{
    return (x >= 0 && y >= 0 && x < maze_game.grid_cols && y < maze_game.grid_rows);
}

size_t MazeExplorerGame::CellIndex(int64_t x, int64_t y) const{
    return static_cast<size_t>(y * maze_game.grid_cols + x);
}

size_t MazeExplorerGame::FlatIndex(const maze_coord_t &coord) const{
    return static_cast<size_t>(coord.floor * maze_game.grid_cols * maze_game.grid_rows
                               + coord.y * maze_game.grid_cols + coord.x);
}

bool MazeExplorerGame::IsWall(int64_t floor_idx, int64_t x, int64_t y) const{
    if(floor_idx < 0 || floor_idx >= maze_game.num_floors) return true;
...
bool MazeExplorerGame::IsStairUp(int64_t floor_idx, int64_t x, int64_t y) const{
    if(floor_idx < 0 || floor_idx >= maze_game.num_floors) return false;
...
MazeExplorerGame::maze_coord_t MazeExplorerGame::RandomWalkableCell(int64_t floor_idx){
    ...
    for(int64_t y = 1; y < maze_game.grid_rows - 1; ++y){
        for(int64_t x = 1; x < maze_game.grid_cols - 1; ++x){
    ...
    return cells[dist(maze_game.re)];
}

std::vector<int64_t> MazeExplorerGame::ComputeDistances(const maze_coord_t &start) const{
    const auto total = static_cast<size_t>(maze_game.num_floors * maze_game.grid_cols * maze_game.grid_rows);
...
void MazeExplorerGame::GenerateMazeFloor(int64_t floor_idx){
    ...
    const auto total = static_cast<size_t>(maze_game.grid_cols * maze_game.grid_rows);
    cells.assign(total, {});
    for(auto &cell : cells){
        cell.wall = true;
        cell.stair_up = false;
    ...
    if(maze_game.grid_cols < 5 || maze_game.grid_rows < 5){
        for(int64_t y = 1; y < maze_game.grid_rows - 1; ++y){
            for(int64_t x = 1; x < maze_game.grid_cols - 1; ++x){
    ...
    std::uniform_int_distribution<int64_t> col_dist(1, std::max<int64_t>(1, (maze_game.grid_cols - 2) / 2));
    std::uniform_int_distribution<int64_t> row_dist(1, std::max<int64_t>(1, (maze_game.grid_rows - 2) / 2));

    int64_t start_col = col_dist(maze_game.re) * 2 - 1;
    int64_t start_row = row_dist(maze_game.re) * 2 - 1;
    start_col = std::clamp<int64_t>(start_col, 1, maze_game.grid_cols - 2);
    start_row = std::clamp<int64_t>(start_row, 1, maze_game.grid_rows - 2);
    ...
    while(!stack.empty()){
        ...
        for(const auto &dir : directions){
            ...
            if(nx > 0 && nx < maze_game.grid_cols - 1 && ny > 0 && ny < maze_game.grid_rows - 1){
        ...
        if(neighbors.empty()){
            ...
            auto [nx, ny] = neighbors[neighbor_dist(maze_game.re)];
...
void MazeExplorerGame::GenerateLevel(){
    ...
    for(int attempt = 0; attempt < 40 && !created; ++attempt){
        floors.clear();
        floors.resize(static_cast<size_t>(maze_game.num_floors));
        for(int64_t floor_idx = 0; floor_idx < maze_game.num_floors; ++floor_idx){
            GenerateMazeFloor(floor_idx);
        }

        for(int64_t floor_idx = 0; floor_idx < maze_game.num_floors - 1; ++floor_idx){
            ...
            if(candidates.empty()){
                std::uniform_int_distribution<int64_t> col_dist(1, maze_game.grid_cols - 2);
                std::uniform_int_distribution<int64_t> row_dist(1, maze_game.grid_rows - 2);
                const auto x = col_dist(maze_game.re);
                const auto y = row_dist(maze_game.re);
            ...
            const auto chosen = candidates[pick(maze_game.re)];
        ...
        const auto target_floor = maze_game.num_floors - 1;
        int64_t max_dist = -1;
        ...
        goal_cell = far_cells[pick(maze_game.re)];
        created = true;
    }
    ...
    if(!created){
        ...
        for(int64_t floor_idx = 0; floor_idx < maze_game.num_floors; ++floor_idx){
            ...
            cells.assign(static_cast<size_t>(maze_game.grid_cols * maze_game.grid_rows), {});
            for(auto &cell : cells){
                cell.wall = false;
                cell.stair_up = false;
                cell.stair_down = false;
        ...
        start_cell = { 0, 1, 1 };
        goal_cell = { maze_game.num_floors - 1, maze_game.grid_cols - 2, maze_game.grid_rows - 2 };
    }
}
...
double MazeExplorerGame::CastRay(const vec2<double> &pos, double angle, int64_t floor_idx, int &hit_side) const{
    ...
    while(distance < maze_game.max_view_distance){
    ...
    if(distance >= maze_game.max_view_distance){
        return maze_game.max_view_distance;
    }
...
bool MazeExplorerGame::Display(bool &enabled){
    ...
    const auto win_width = static_cast<int>(std::ceil(maze_game.box_width)) + 40;
    const auto win_height = static_cast<int>(std::ceil(maze_game.box_height)) + 140;
    ...
    ImVec2 view_size(maze_game.box_width, maze_game.box_height);
    ...
    if(focused){
        ...
        if(ImGui::IsKeyDown(SDL_SCANCODE_LEFT)) player_angle -= maze_game.rot_speed * dt;
        if(ImGui::IsKeyDown(SDL_SCANCODE_RIGHT)) player_angle += maze_game.rot_speed * dt;
        ...
        if(move_len > 1.0e-4){
            move_dir.x /= move_len;
            move_dir.y /= move_len;
            const auto delta = move_dir * (maze_game.move_speed * dt);
        ...
        if(ImGui::IsKeyPressed(SDL_SCANCODE_E) && IsStairUp(player_floor, cell_x, cell_y)){
            player_floor = std::min<int64_t>(maze_game.num_floors - 1, player_floor + 1);
        }
        if(ImGui::IsKeyPressed(SDL_SCANCODE_Q) && IsStairDown(player_floor, cell_x, cell_y)){
            player_floor = std::max<int64_t>(0, player_floor - 1);
        }

        if(ImGui::IsKeyPressed(SDL_SCANCODE_SPACE) && !maze_game.is_jumping){
            maze_game.is_jumping = true;
            maze_game.jump_velocity = maze_game.jump_speed;
        }
    }

    if(maze_game.is_jumping){
        maze_game.jump_velocity -= maze_game.gravity * dt;
        maze_game.jump_height += maze_game.jump_velocity * dt;
        if(maze_game.jump_height <= 0.0){
            maze_game.jump_height = 0.0;
            maze_game.jump_velocity = 0.0;
            maze_game.is_jumping = false;
        }
    ...
    if(moved){
        maze_game.walk_phase += dt * 2.2;
    }else{
        maze_game.walk_phase *= std::pow(0.85, dt * 60.0);
    }
    ...
    if(!maze_game.level_complete && player_floor == goal_cell.floor && dist_to_goal < 0.5){
        maze_game.level_complete = true;
        maze_game.completion_time = elapsed;
    }

    const auto bob = std::sin(maze_game.walk_phase * 2.0 * pi) * (moved ? 1.0 : 0.25);
    const auto vertical_offset = static_cast<float>((-maze_game.jump_height * 0.35 + bob * 0.06) * view_size.y);
    ...
    for(int ray = 0; ray < ray_count; ++ray){
        const auto ray_angle = player_angle - maze_game.fov * 0.5
                             + (static_cast<double>(ray) + 0.5) * (maze_game.fov / ray_count);
        int hit_side = 0;
        ...
        const auto shade = std::clamp(1.0 - dist / maze_game.max_view_distance, 0.2, 1.0);
    ...
    if(player_floor == goal_cell.floor){
        ...
        if(std::abs(rel_angle) < maze_game.fov * 0.5 && dist_to_goal < maze_game.max_view_distance){
            int hit_side = 0;
            const auto wall_dist = CastRay(player_pos, angle_to_goal, player_floor, hit_side);
            if(wall_dist >= dist_to_goal - 0.1){
                const auto screen_x = view_pos.x + (static_cast<float>(rel_angle / maze_game.fov) + 0.5f) * view_size.x;
    ...
    const auto timer_value = maze_game.level_complete ? maze_game.completion_time : elapsed;
    std::stringstream ss;
    ss << "Time: " << std::fixed << std::setprecision(1) << timer_value << "s";
    ss << "  Floor " << (player_floor + 1) << "/" << maze_game.num_floors;

    if(maze_game.level_complete){
        ss << "  Level complete!";
    }
    ...
    if(stair_up || stair_down){
        ...
        draw_list->AddText(ImVec2(view_pos.x + 10.0f, view_pos.y + 30.0f),
                           ImColor(0.9f, 0.9f, 0.6f, 1.0f), stair_text);
    }

    const auto figure_origin = ImVec2(view_pos.x + view_size.x - 70.0f, view_pos.y + view_size.y - 90.0f);
    const auto leg_swing = static_cast<float>(std::sin(maze_game.walk_phase * 2.0 * pi) * 6.0);
    const auto jump_lift = static_cast<float>(maze_game.jump_height * 18.0);
    ...
    const auto arm_left = ImVec2(head_center.x - 10.0f, head_center.y + 16.0f - (maze_game.is_jumping ? 4.0f : 0.0f));
    const auto arm_right = ImVec2(head_center.x + 10.0f, head_center.y + 16.0f - (maze_game.is_jumping ? 4.0f : 0.0f));
