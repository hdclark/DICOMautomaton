...
class TacticalStealthGame {
  ...
  private:
    ...
    struct ts_game_t {
        ...
        std::vector<bool> walls; // true = wall, false = walkable
        ...
        double base_fov_range = 120.0;   // Base field of view range in pixels
        double hearing_range = 50.0;      // Base range to hear player
        double pursuit_speed_mult = 2.0;  // Speed multiplier when pursuing
        ...
        // Level scaling
        double speed_scale_per_level = 0.2;  // 20% increase per level
        double fov_scale_per_level = 0.2;    // 20% increase per level
