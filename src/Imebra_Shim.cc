    ...
    do{
         ...
         if(SecondDataSet != nullptr){
            ...
            do{
                ...
                if(ROI_name.size()){
                    if(VERBOSE) FUNCINFO("The ROI name   for item # " << i << " " << j << " is " << ROI_name); 
                    if(VERBOSE) FUNCINFO("The ROI number for item # " << i << " " << j << " is " << ROI_number);
...
std::unique_ptr<Pixel_Data>  get_pixel_data_and_numbers(std::string FilenameIn){
    std::unique_ptr<Pixel_Data> output (new Pixel_Data());

    using namespace puntoexe;  //Used to shorten puntoexe::ptr because it is annoying.
    ptr<puntoexe::stream> readStream(new puntoexe::stream);
    readStream->openFile(FilenameIn.c_str(), std::ios::in);
    ...
    output->filenames.push_back(FilenameIn);
    ...
    output->pixel_dz     = static_cast<double>( TopDataSet->getDouble( 0x0018, 0, 0x0050, 0 ) );
    output->slice_height = static_cast<double>( TopDataSet->getDouble( 0x0020, 0, 0x1041, 0 ) );

    //These are Dose-only.
    output->grid_scale   = static_cast<double>( TopDataSet->getDouble( 0x3004, 0, 0x000e, 0 ) );
    ...
    output->image_pos_y = static_cast<double>( TopDataSet->getDouble( 0x0020, 0, 0x0032, 0 ) );
    output->image_pos_x = static_cast<double>( TopDataSet->getDouble( 0x0020, 0, 0x0032, 1 ) );
    output->image_pos_z = static_cast<double>( TopDataSet->getDouble( 0x0020, 0, 0x0032, 2 ) );

    
    //First, determine how many frames there are in the pixel data. A CT scan may just be a 2d jpeg or something, 
    // but dose pixel data is 3d data composed of 'frames' of stacked 2d data.
    long int frame_count = static_cast<long int>( TopDataSet->getUnsignedLong( 0x0028, 0, 0x0008, 0 ) );
    if(frame_count == 0) frame_count = 1;
    if(VERBOSE) FUNCINFO(" Found " << frame_count << " frames in file: " << FilenameIn );

    //Grab the image data for each individual frame. Let's hope Imebra is not leaky...
    for(unsigned int curr_frame = 0; static_cast<long int>(curr_frame) < frame_count; ++curr_frame ){  //Dunno how Imebra deals with its int/uint conversions, so keep curr_frame unsigned..
        //Now we retrieve the pixel data from file. This is an excessively long procedure! ------------------------
        ptr<puntoexe::imebra::image> firstImage = TopDataSet->getImage( curr_frame ); 	
   
        if(firstImage == NULL) FUNCERR("This file does not have accessible pixel data. Double check the file");
    
        //Note that an image should be processed by the modalityVOILUT transform to convert its pixel values into meaningful values.
        ...
        //Further processing may be required to convert the convertedImage into an image suitable for the presentation on the screen. 
        //The VOILUT transform applies the contrast suggested by the dataSet to the image. For instance:
        ...
        puntoexe::imebra::transforms::colorTransforms::colorTransformsFactory*  pFactory = puntoexe::imebra::transforms::colorTransforms::colorTransformsFactory::getColorTransformsFactory();
        ptr<puntoexe::imebra::transforms::transform> myColorTransform = pFactory->getTransform(presentationImage->getColorSpace(), L"RGB");
        // color transform not needed if the factory returns 0
        if(myColorTransform != 0){
            ptr<puntoexe::imebra::image> rgbImage(myColorTransform->allocateOutputImage(presentationImage, width, height));
        ...
        ptr<puntoexe::imebra::handlers::dataHandlerNumericBase> myHandler = presentationImage->getDataHandler(false, &rowSize, &channelPixelSize, &channelsNumber);
    
        // Retrieve the image's size in pixels
        imbxUint32 sizeX, sizeY;
        presentationImage->getSize(&sizeX, &sizeY);
    
        if( output->data == NULL ){  //If we have not yet allocated memory, we must be on the first pass..
    
            //First we will read metadata about the pixel data. We will use this to allocate space to a new
            // pointer to the data. Afterward, we will transfer ownership of the pixel data to the pointer 
            // inside the struct, and then ship it out.
            //struct Pixel_Data output;
            output->rows     = static_cast<long int>( TopDataSet->getUnsignedLong( 0x0028, 0, 0x0010, 0 ) );
            output->columns  = static_cast<long int>( TopDataSet->getUnsignedLong( 0x0028, 0, 0x0011, 0 ) );
            output->height   = static_cast<long int>( frame_count );
            
            output->pixel_dx = TopDataSet->getDouble( 0x0028, 0, 0x0030, 0 );
            output->pixel_dy = TopDataSet->getDouble( 0x0028, 0, 0x0030, 1 );
            //output->pixel_dz = ;

            output->channels = static_cast<unsigned int>( channelsNumber ); //RGB = 3, Monochrome = 1, RGBA = 4, etc..
            output->bits     = static_cast<unsigned int>( channelPixelSize*8 );   //16 bit, 32 bit, 8 bit, etc..

            output->max.reset( new unsigned int [output->channels] );  //Used for display purposes - helps to adjust contrast when viewing.
            if(output->max == NULL) FUNCERR("Unable to allocate space for per-channel maximum pixel value.");    
            for(size_t i=0; i<output->channels; ++i){
                output->max[i] = 0;
            }  

            //Verify that the two readings provide identical information (dataHandler and direct tag reading..)
            if( (output->rows == static_cast<long int>(sizeY))  &&  (output->columns == static_cast<long int>(sizeX)) ){  
                //We should only consume an amount of data which scales with the number of bits used - but we assume whatever value the pixel data
                // is will be typecast into a 32bit unsigned int. 
                if(VERBOSE) FUNCINFO("Allocating " << (long int)(output->channels*output->rows*output->columns*output->height) << " long unsigned ints");

//                output->data.reset( new unsigned int [output->channels*output->rows*output->columns*output->height*output->bits] );    //      <---------- This seems to work but chokes on medium to large file sizes. Maybe I do not need the number of bits??
                output->data.reset( new unsigned int [output->channels*output->rows*output->columns*output->height] );        



                if(output->data == NULL){
                    FUNCERR("Unable to allocate memory of size: [" << output->channels * output->rows * output->columns * output->height << "]");
                    //NOTE: It might be the case that sizeX == columns and sizeY == rows. Check this case before any other
                    // troubleshooting. (This is still a DICOM file error, but it is probably not so bad to proceed anyways.)
                }
            }else{
                output->data = NULL; //(Should already be NULL, but meh...)
                FUNCWARN("DataHandler says: " << sizeX << " x " << sizeY << "  but reading directly says: " << output->rows << " x " << output->columns << ".");
                FUNCERR("DICOM file gives inconsistent data. Please verify row, column pixel numbers.");
            }
        }
        ...
        for(imbxUint32 scanY = 0; scanY < sizeY; ++scanY){
            ...
            for(imbxUint32 scanX = 0; scanX < sizeX; ++scanX){
                ...
                for(imbxUint32 scanChannel = 0; scanChannel < channelsNumber; ++scanChannel){
                    ...
                    //imbxInt32 channelValue = myHandler->getSignedLong( data_index );
                    unsigned int channelValue = static_cast<unsigned int>( myHandler->getUnsignedLong( data_index ) );
                    output->data[output->channels*output->rows*output->columns*curr_frame + data_index] = channelValue;  //Data is likely to be an unsigned short: [0:65535]
   
                    if( channelValue > output->max[scanChannel] ) output->max[scanChannel] = channelValue;
                    //Data is likely to be an unsigned short (16 bit - [0:65535]), but by embedding the numerical
                    // number in an unsigned int (32 bit), we are safe to put whatever we want (up to 32 bits) in.
                    // We can cast the data later (the actual bit size is stored in the Pixel_Data struct), but
                    // this method provides a simple way of treating all possible bit sizes (up to 32 bit) in one 
                    // go without resorting to templates or ____ .
    
                    //NOTE: Dose data is *probably* going to be 32 bit!
                    ++data_index;
        ...
        if(VERBOSE) FUNCINFO("Filled array with " << (long int)(data_index) << " long unsigned ints");
...
//Attempts to amalgamate data from several input files. They are stacked on top of one another to produce a 
// 3d matrix of the data. An attempt is made to order them, but the burden is on the user to provide a 
// suitable list of filenames, preferably (but not necessarily) in a vertically-stacked order.
std::unique_ptr<Pixel_Data> get_pixel_data_and_numbers(std::vector<std::string> FilenameIn){
    //It is assumed all files exist. No recovery happens if a file is absent - the routine 
    // (nicely) kills the program!
    ...
    //First, grab pixel metadata. Row #'s, channels, etc.. Everything except the (expensive) pixel data
    // and per-pixel information.
    std::vector< std::unique_ptr<Pixel_Data> > metadata;
    for(size_t i=0; i<FilenameIn.size(); ++i){
        std::unique_ptr< Pixel_Data > temp = estimate_pixel_memory( FilenameIn[i] );
        metadata.push_back( std::move( temp ) );
    }

    //Now we sort the metadata and filenames by the slice_height parameter.
    auto Pixel_Data_Height_Sort_Lambda = [](std::unique_ptr<Pixel_Data> const &a, std::unique_ptr<Pixel_Data> const &b){ return a->slice_height > b->slice_height; };
    sort(metadata.begin(), metadata.end(), Pixel_Data_Height_Sort_Lambda);

    //Compute the expected number of (unsigned int)s we will need.  [Not bytes!]
    unsigned long int total = 0;
    for(size_t i=0; i<metadata.size(); ++i){
        if(VERBOSE) FUNCINFO(" The image data in file " << i << " has " << metadata[i]->rows << " rows, " << metadata[i]->columns << " columns, " << metadata[i]->height << " height, " << metadata[i]->channels << " channels, and " << metadata[i]->bits << "-bit values.");
//        if( !((metadata[i]->bits == 32) || (metadata[i]->bits == 8) || (metadata[i]->bits == 16)) ) FUNCERR("Only prepared to handle 32, 8, and 16 bit data. Please fix me first.");
        total += metadata[i]->height * metadata[i]->rows * metadata[i]->columns * metadata[i]->channels;

        output->height   += metadata[i]->height;  // (Each 2d image might actually be a stack of 2d images.)

        //Set some metadata for the 3d data OR query for consistency.
        if(i==0){
            output->rows     = metadata[i]->rows;
            output->columns  = metadata[i]->columns;

            output->channels = metadata[i]->channels;
            output->bits     = metadata[i]->bits;

            output->pixel_dx = metadata[i]->pixel_dx;
            output->pixel_dy = metadata[i]->pixel_dy;
            output->pixel_dz = metadata[i]->pixel_dz;

            output->slice_height = metadata[i]->slice_height; //See below! (in the else)

            output->image_pos_x = metadata[i]->image_pos_x;
            output->image_pos_y = metadata[i]->image_pos_y;
            output->image_pos_z = metadata[i]->image_pos_z; //Arbitrarily pick this one.

            output->grid_scale  = metadata[i]->grid_scale;

            output->filenames.insert( output->filenames.end(), metadata[i]->filenames.begin(), metadata[i]->filenames.end() ); //append.

        }else{
           
            if( output->rows     != metadata[i]->rows     ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 01");
            if( output->columns  != metadata[i]->columns  ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 02");
            
            if( output->channels != metadata[i]->channels ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 03");
            if( output->bits     != metadata[i]->bits     ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 04");
            
            if( output->pixel_dx != metadata[i]->pixel_dx ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 05");
            if( output->pixel_dy != metadata[i]->pixel_dy ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 06");
            if( output->pixel_dz != metadata[i]->pixel_dz ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 07");


            if( output->image_pos_x != metadata[i]->image_pos_x ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 08");
            if( output->image_pos_y != metadata[i]->image_pos_y ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 09");
//            if( output->image_pos_z != metadata[i]->image_pos_z ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 10");

            if( output->grid_scale  != metadata[i]->grid_scale ) FUNCERR("Unamalgamatable pixel data. Unable to amalgamate 11");

            //This is not a typo. It is the laziest way I could think of to grab the last value.
            //Why this one? Because it matches up with the contour information!
//            output->slice_height = metadata[i]->slice_height; 

        }
    }

    if(VERBOSE) FUNCINFO("The total number of bytes we will need to allocate is " << total );

    //Now we allocate this space. It is probably huge. If nothing else, I would worry about the gravitational
    // pull it will likely impose on neighbouring stars. Basically - keep an eye out for withchunts and angry
    // mobs if you use this function.
    output->max.reset( new unsigned int [output->channels] );  //Used for display purposes - helps to adjust contrast when viewing.
    if(output->max == NULL) FUNCERR("Unable to allocate space for per-channel maximum pixel value.");
    for(size_t i=0; i<output->channels; ++i){
        output->max[i] = 0;
    ...
    //Are you sweating? I am. Check your memory usage, sucka!


    //Now we grab the actual data and copy it into the mega-buffer. We do this one at a time to try keep memory usage down.
    unsigned long int dumped = 0;
    for(size_t I=0; I<metadata.size(); ++I){
        for(size_t J=0; J<metadata[I]->filenames.size(); ++J){ //Should just be one..

            std::unique_ptr<Pixel_Data> temp = get_pixel_data_and_numbers( metadata[I]->filenames[J] );
    
            //I realize this is gross, but I do not want to just copy memory over. Perhaps when I later optimize and the bugs
            // are less numerous. 
            for(unsigned long int i=0; i<(temp->rows*temp->columns*temp->channels*temp->height); ++i ){
                output->data[dumped] = temp->data[i];
                ++dumped;
            }
       
            //Nab the maximum pixel data the easy way.
            for(size_t j=0; j<output->channels; ++j){
                if(output->max[j] < temp->max[j]) output->max[j] = temp->max[j];
            }
        }
        output->filenames.insert( output->filenames.end(), metadata[I]->filenames.begin(), metadata[I]->filenames.end() ); //append.
    }

    if(dumped != total) FUNCERR("Incorrectly computed total number of bytes required to store the 3d data: " << dumped << " " << total );
